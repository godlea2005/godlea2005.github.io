...
describe('commerce administrator console operations', () => {
  ...
  it('uses readable administrator metadata tokens and preserves a visible tabpanel focus ring', () => {
    ...
    expect(css).toMatch(/\.commerce-admin-section-header span,[\s\S]*\.commerce-admin-cleanup > span \{[^}]*font: 10px/)
    expect(css).toMatch(/\.commerce-admin-validation \{[^}]*font-size: 10px/)
    expect(css).toMatch(/\.commerce-admin-dialog > small \{[^}]*font-size: 10px/)
