...
export function CommerceRunPanel({ step, productName, platformName, modeName, imageCount, creditsLabel, state, formValidity, requirements, onNext, onRetry, onRecoverAuthentication, onRecovery }: CommerceRunPanelProps) {
  ...
  return <aside className="commerce-run-panel commerce-summary" aria-label="当前分析摘要">
    ...
    {action === 'progress' ? <button data-commerce-primary-action className="commerce-submit" type="button" disabled><span>{presentation.primaryLabel}</span><i aria-hidden="true">•••</i></button>
      : action === 'retry' || action === 'reauthenticate' || action === 'restart' ? <div className="commerce-submit-error" role="alert"><strong>这一步没有完成</strong><p>{error}</p>{action === 'retry' ? <button data-commerce-primary-action type="button" onClick={onRetry}>重试本次生成</button> : action === 'reauthenticate' ? <button data-commerce-primary-action type="button" onClick={onRecoverAuthentication}>重新连接账号</button> : <button data-commerce-primary-action type="button" onClick={onRecovery}>返回修改资料</button>}</div>
        : action === 'view-result' ? <p className="commerce-complete-note" role="status">结果已安全写入，可进入方案页查看。</p>
          : action === 'recover-result' ? <button data-commerce-primary-action className="commerce-submit" type="button" onClick={onRecovery}><span>{presentation.primaryLabel}</span><i aria-hidden="true">←</i></button>
            : <><p className="commerce-requirements" id="commerce-submit-requirements" role="status">{readyCopy}</p>{step < 3 ? <button data-commerce-primary-action className="commerce-submit" type="button" onClick={onNext}><span>下一步</span><i aria-hidden="true">→</i></button> : <button data-commerce-primary-action className="commerce-submit" type="submit" disabled={presentation.disabled} aria-describedby="commerce-submit-requirements commerce-submit-hint"><span>{presentation.primaryLabel}</span><i aria-hidden="true">↗</i></button>}<small id="commerce-submit-hint">确认提交后消耗 1 次；服务失败会自动退款。</small></>}
  </aside>
}
